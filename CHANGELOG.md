# Changelog

所有重要变更将记录在此文件中。

## [1.4.0] - 2026-07-31

### 新增
- **预算智能建议系统**：新增 `references/budget-advisor.md`，基于关系阶段 × 关系深度 × 场景 × 历史记录的公式化预算建议，覆盖 L1-L6 六档预算与品类匹配
- **季节性送礼指南**：新增 `references/seasonal-guide.md`，四季花卉推荐、节日日历、特殊天气适配、季节×阶段交叉矩阵
- **多品类对比推荐**：每个场景推荐 3-5 种不同品类（鲜花+首饰+体验+定制等），横向对比给用户更多选择
- **报告模板升级 v2**：摘要区新增预算建议区块、季节适配区块、多品类对比网格；卡片新增季节适配理由、预算合理性分析
- **推荐理由扩展至 6 维度**：新增季节适配理由（引用 seasonal-guide.md）

### 优化
- `relationship-stages.md` 所有阶段更新为 5 种品类推荐格式（带 emoji 图标和价格区间）
- `SKILL.md` 集成预算建议、季节适配、品类多样化工作流
- `html-spec.md` 新增 7 个模板变量说明

## [1.3.0] - 2026-07-31

### 新增
- **关系阶段推荐引擎**：新增 `references/relationship-stages.md`，定义六大关系阶段（追求期/热恋期/稳定期/新婚期/周年纪念/长期陪伴）的送礼策略
- **花语知识库**：新增 `references/flower-guide.md`，包含完整的花语速查、朵数含义、场合×阶段送花矩阵、推荐组合方案、避坑提醒
- **阶段评分维度**：评分模型新增"阶段适配度"维度（20% 权重），按关系阶段评估品类、预算、情感浓度匹配度
- **画像 Schema 扩展**：`profile-schema.md` 新增 `relationship_stage` 和 `relationship_start_date` 字段
- **报告模板升级**：HTML 模板新增阶段徽章、阶段适配度评分条、阶段理由区块、花语/朵数展示区块
- **推荐理由生成规则**：每个候选必须包含 5 个维度的推荐理由（阶段适配、画像匹配、花语寓意、场景适配、价格合理性）
- **SKILL.md 工作流集成**：阶段 0 增加关系阶段确认、阶段 1 增加阶段过滤规则、阶段 4 增加推荐理由生成规则、资源索引新增 2 个文档

### 优化
- 评分权重预设更新为五维（新增"浪漫优先"预设，阶段适配权重 45%）
- 雷达图标签更新为五维对比

## [1.2.0] - 2026-07-31

### 新增
- **Playwright 登录态管理**：新增 `scripts/login_manager.py`，基于 Playwright 持久化浏览器上下文实现登录态管理
  - `login` 命令：启动有头浏览器让用户手动登录，自动检测成功并保存状态
  - `check` 命令：无头模式校验登录态有效性
  - `list` 命令：列出所有已保存的登录态
  - `clear` 命令：清除指定渠道的登录态
  - 支持淘宝、京东、小红书三个渠道
  - 自动检测正向/负向登录信号（URL、DOM元素、文本内容）
- **SKILL.md 集成**：登录态检查流程从依赖 browser-use 改为 Playwright login_manager.py
- **data-sources.md 重写**：登录态技术实现全面更新为 Playwright 架构
- **.gitignore 更新**：增加 `sessions/` 目录排除

## [1.1.0] - 2026-07-30

### 新增
- **画像 Schema 版本管理**：新增 `schema_version` 字段和 `migrate` 命令，支持旧版画像自动迁移
- **环境变量配置**：存储路径可通过 `GIFT_PICKER_HOME` 环境变量覆盖
- **评分权重预设**：新增 4 种评分偏好预设（均衡/性价比优先/品质优先/口碑优先），用户可按需选择
- **登录态技术文档**：`data-sources.md` 新增完整的登录态底层机制、校验流程和熔断策略说明
- **断点 JSON 化**：`memory.md` 升级为 `memory.json`，结构化存储任务状态，支持可靠的断点恢复
- **离线兜底**：HTML 模板在 Chart.js 加载失败时显示友好提示

### 修复
- **P0 安全：路径遍历防护**：`_path()` 增加 `..` 和点前缀检查，限制昵称长度 50 字符
- **P0 安全：XSS 防护**：HTML 模板注释明确要求所有占位符数据必须 HTML 转义；模板内嵌图片加载失败的 SVG fallback
- **P0 数据：写入备份**：`cmd_set()` 写入前自动备份，写入失败自动恢复
- **P0 数据：原子写入**：用 `tempfile` + `os.replace` 实现原子写入，防止中断损坏
- **P1 健壮：错误处理**：`_atomic_write()` 捕获 OSError，`cmd_set()` 捕获 json-file 读取异常
- **P1 逻辑：深度合并**：`_deep_merge()` 对 `colors_like`、`allergies` 等数组字段采用 extend+去重合并，而非整体替换
- **P1 逻辑：cmd_list 简化**：消除重复的目录存在性检查
- **P2 体验：图片 fallback**：`onerror` 替换为 SVG 占位图 + CSS 错误样式

## [1.0.0] - 2026-07-29

### 初始版本
- 恋人画像管理（持久化 + 增量合并）
- 社媒 ID 一键画像（小红书/微博/B站/抖音）
- 多渠道采集（小红书口碑 + 搜索引擎 + 淘宝/京东）
- 四维评分模型（价格/质量/好评/差评反向）
- HTML 礼物清单报告（含雷达图 + 多渠道比价 + AI 分析）
- Agent Skills 开放标准兼容