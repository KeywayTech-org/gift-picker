...
### 小红书（口碑层）
- 必须实际打开小红书公开搜索页或笔记页。每个候选品类执行种草与避坑两组检索：`<品类> 送女友`、`<品牌><品类> 真实测评`、`<品类> 踩雷`。
- 每个品类至少保存 2 篇可访问笔记的 URL、标题和可见互动数据；从正文或评论提炼至少 2 条购买信号和 1 条避坑信号。无来源 URL 的内容不能标为小红书口碑。
- 在 `memory.json` 写入 `xiaohongshu.status`：`opened`（已读取笔记/评论）、`fallback`（仅公开检索快照）或 `unavailable`（无法取得可读内容），并保存笔记 URL 与提炼结果。
- 登录说明：小红书公开内容免登可读。遇到登录、风控或页面不可访问时，只执行一次 `site:xiaohongshu.com` 公开检索作为降级；仍无结果则为 `unavailable`，报告必须注明“小红书口碑未获取”并降低口碑置信度，禁止静默跳过。
