---
name: gift-picker
description: 礼物挑选助手。为女朋友/老婆/心仪对象挑选礼物。工作流必须依次完成 5 步提问：①画像描述 → ②关系阶段 → ③预算 → ④场景品类 → ⑤登录态，每步必做，不可跳过。能力：画像持久化、多渠道商品搜索、五维评分、阶段适配、预算建议、季节适配、HTML 报告。
agent_created: true
---
...
用 AskUserQuestion 问："还没她的画像，怎么开始？"
选项：
- 🔗 社媒 ID 一键画像（推荐）
- 📸 发截图给我
- ✏️ 一段话描述
...
| 优先级 | 方式 | 操作 |
|--------|------|------|
| **1（最高）** | 社媒 ID 一键画像 | 用 browser-use 读取小红书/微博/抖音/B站号公开主页 → 自动检索公开媒体 → 喜好分析 → **对话内输出不落盘的喜好报告** → 用户确认后 `set` 保存。详见 `references/social-profiling.md` |
| **2** | 截图解析 | 解析小红书收藏/购物车/穿搭照等图片，提取色系/品牌/风格关键词，逐项映射到画像字段 |
| **3** | 一段话速写 | AI 解析用户描述为结构化字段，置信度低的标注"推测" |
| **4（兜底）** | 逐题问答 | 仅补必答缺口（过敏忌口），用 AskUserQuestion，每次 1 题，2-4 个默认选项 |
...
结合 `relationship_stage` 过滤品类（详见 `references/relationship-stages.md`），每个场景推荐 3-5 种不同品类。创建 `memory.json` 写入任务头。

### Step 3：登录态确认与自动跳转登录

用 AskUserQuestion 问："是否用登录态获取京东/淘宝真实商品链接？"
选项：淘宝 / 京东 / 淘宝+京东 / 跳过（降级模式）

按用户选择，对每个渠道执行以下流程：

1. 运行 `python scripts/login_manager.py check_and_login <channel>`
2. 该命令会自动：
   - 有有效登录态 → ✅ 输出"登录态有效"，直接进入采集
   - 无登录态或已失效 → 🌐 **自动打开浏览器跳转到淘宝/京东登录页**
3. 浏览器打开后，告诉用户：
   ```
   🌐 已为你打开 [淘宝/京东] 登录页，请在浏览器中完成登录（扫码或账号密码）
   ⏰ 登录成功后状态将自动保存，下次使用无需重新登录
   ```
4. 等待 `check_and_login` 脚本返回成功（或超时），确认登录完成后继续

跳过 → 仅用搜索引擎+小红书，报告标注"无购买链接"
